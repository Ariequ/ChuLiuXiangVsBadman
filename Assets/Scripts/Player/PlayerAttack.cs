...
public class PlayerAttack : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		else if (stateInfo.IsName(AtkStabState) && (stateInfo.normalizedTime > 0.8f) && (this.curComboCount == 3))
		{
			// 挡在攻击2状态下（同理攻击1状态）
			this.animator.SetInteger(ActionCMD, 1);
			canMakeDamage = true;
		...
		else if (stateInfo.IsName(AtkCleave) && (stateInfo.normalizedTime > 0.8f) && (this.curComboCount == 4))
		{
			// 挡在攻击3状态下（同理攻击1状态）
			this.animator.SetInteger(ActionCMD, 1);
			canMakeDamage = true;
